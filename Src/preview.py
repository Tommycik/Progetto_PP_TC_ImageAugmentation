...
def _open_folder(path: Path) -> None:
    # Open the preview folder automatically on Windows when possible.
    if os.name == "nt":
        try:
            os.startfile(path)
        except OSError:
...
def _run_single_backend(
    ...
    # Run one backend only and return a labelled batch for the preview sheet.
    profile = PROFILES[profile_name]
    batch_size = len(images)
    resolution = images[0].shape[0]

    if backend_name == "albumentations_sequential":
        prepared = prepare_albumentations(profile, batch_size)
        return "Albumentations sequential", run_sequential(images, prepared)

    if backend_name == "albumentations_threaded":
        prepared = prepare_albumentations(profile, batch_size)
        worker_count = min(12, max(1, batch_size))
        with ThreadPoolExecutor(max_workers=worker_count) as executor:
            return f"Albumentations {worker_count} threads", run_threaded(images, prepared, executor)

    if backend_name == "kornia_cpu":
        cpu_tensor = numpy_batch_to_tensor(images)
        parameters = prepare_kornia_parameters(
            profile, batch_size, resolution, resolution, torch.device("cpu")
        )
        return "Kornia CPU", tensor_to_numpy_batch(apply_kornia(cpu_tensor, parameters))

    if backend_name == "kornia_cuda":
        if not torch.cuda.is_available():
            raise RuntimeError("Kornia CUDA preview is unavailable because PyTorch CUDA is not available.")
        cpu_tensor = numpy_batch_to_tensor(images)
        device = torch.device("cuda:0")
        parameters = prepare_kornia_parameters(
            profile, batch_size, resolution, resolution, device
        )
        output = apply_kornia(cpu_tensor.to(device), parameters)
        torch.cuda.synchronize()
        return "Kornia CUDA", tensor_to_numpy_batch(output)

    if backend_name == "cupy_cuda_16x16":
        if not cupy_cuda_available():
            raise RuntimeError("CuPy preview is unavailable because CuPy CUDA is not available.")
        parameters = prepare_cupy_parameters(profile, batch_size, resolution, resolution)
        cupy_input = numpy_batch_to_cupy(images)
        workspace = create_workspace(batch_size, resolution, resolution)
        cupy_output = apply_cupy_device(cupy_input, parameters, workspace, (16, 16))
        synchronize_cupy()
        return "CuPy CUDA block 16x16", cupy_to_numpy_batch(cupy_output)

    raise ValueError(f"Unsupported preview backend: {backend_name}")


def _automatic_fast_backend() -> str:
    # Choose a default backend
    if torch.cuda.is_available():
        return "kornia_cuda"
    if cupy_cuda_available():
        return "cupy_cuda_16x16"
    return "albumentations_threaded"
...
def generate_previews(
    ...
    *,
    ...
    # Generate previews for the selected profile.
    ...
    # The same resized batch is reused for every preview backend.
    images = build_batch(source_images, resolution, batch_size)
    groups: list[tuple[str, list[np.ndarray]]] = [("Input", images)]

    if mode == "fast":
        chosen_backend = _automatic_fast_backend()
        label, preview_images = _run_single_backend(images, profile_name, chosen_backend)
        groups.append((label, preview_images))
        print(f"Fast preview backend: {label}")
        print("Only the selected backend was executed.")
        print("No sequential reference was generated.")

    elif mode == "specific":
        chosen_backend = backend_name or "albumentations_threaded"
        label, preview_images = _run_single_backend(images, profile_name, chosen_backend)
        groups.append((label, preview_images))
        print(f"Specific preview backend: {label}")
        ...
        cupy_images: list[np.ndarray] = []
        if cupy_cuda_available():
            cupy_parameters = prepare_cupy_parameters(profile, batch_size, resolution, resolution)
            cupy_input = numpy_batch_to_cupy(images)
            cupy_workspace = create_workspace(batch_size, resolution, resolution)
            cupy_output = apply_cupy_device(cupy_input, cupy_parameters, cupy_workspace, (16, 16))
            synchronize_cupy()
            cupy_images = cupy_to_numpy_batch(cupy_output)
            groups.append(("CuPy CUDA block 16x16", cupy_images))

        kornia_cuda: list[np.ndarray] = []
        if torch.cuda.is_available():
            device = torch.device("cuda:0")
            gpu_parameters = prepare_kornia_parameters(profile, batch_size, resolution, resolution, device)
        ...
        # Print a compact numerical comparison to help the visual inspection.
    ...
    # Save every generated image so the user can inspect individual outputs.
